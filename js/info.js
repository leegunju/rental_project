	...
	const sceneInfo = [
		{
			...
			objs: {
				...
				okLogo: document.querySelector('#scroll-section-0 .ok-logo'),
			...
			values: {
				...
				okLogo_width_in: [1000, 200, { start: 0.1, end: 0.4 }],
				okLogo_width_out: [200, 50, { start: 0.4, end: 0.8 }],
				okLogo_translateX_in: [-10, -20, { start: 0.2, end: 0.4 }],
				okLogo_translateX_out: [-20, -50, { start: 0.4, end: 0.8 }],
				okLogo_opacity_out: [1, 0, { start: 0.8, end: 0.9 }],
	...
	function playAnimation() {
		...
		switch (currentScene) {
			case 0:
				...
				if (scrollRatio <= 0.4) {
					objs.okLogo.style.width = `${calcValues(values.okLogo_width_in, currentYOffset)}vw`;
					objs.okLogo.style.transform = `translate(${calcValues(values.okLogo_translateX_in, currentYOffset)}%, -50%)`;
				} else {
					objs.okLogo.style.width = `${calcValues(values.okLogo_width_out, currentYOffset)}vw`;
					objs.okLogo.style.transform = `translate(${calcValues(values.okLogo_translateX_out, currentYOffset)}%, -50%)`;
				}
				...
				objs.okLogo.style.opacity = calcValues(values.okLogo_opacity_out, currentYOffset);
...
var mybutton = document.getElementById("goToPage");
...
window.onscroll = function() {
    if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight) {
        ...
        mybutton.style.transform = "translate(-50%, 100%)";
